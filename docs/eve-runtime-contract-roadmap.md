...
| RC-13 | Add velocity and TAA | Partial | RC-05D, RC-07, RC-12B1 | Native High TAA, four-phase jitter, retained input/output/cooldown products, camera/object history, and raw velocity/cooldown readback are operational. Static and post-frame-180 camera tests plus the integrated composition pass. CPU matrix-reference error bounds, edge-variance reduction, and quantitative Silk/engine residual-energy gates remain before acceptance. |
...
| CP-29 | V5 camera/object velocity observability | Accepted as capability | `VelocityMap` is retained as full-resolution `R16G16_FLOAT`; raw readback converts clip displacement to pixels. Static motion is exactly zero. After the frame-180 camera gate, 1,227,193 of 1,228,800 pixels move with mean `3.25269` and maximum `4.82236` pixels, with no invalid values. | A depth/matrix CPU reference and discontinuity-masked error percentiles remain required for RC-13 acceptance. |
| CP-30 | Native High TAA and transient cooldown | Accepted as capability | The native TAA/TAACopy passes retain FP16 input/output, RGBA16 accumulation, and an integer cooldown map. One reset, four distinct jitter phases, `0.96` maximum blend, and `0.001` threshold validate. The integrated Silk/engine frame reports 19,931 cooldown pixels in `[0,185]`. | Edge-variance reduction and previous-only transient residual energy are not yet measured; manual ghosting acceptance also remains. |
...
| Rung 7: velocity and temporal AA | Enabling TAA did not prove history correctness or transient rejection | Require fixed-ship camera motion after 180 still frames, raw velocity and integer cooldown observability, frozen-history-safe captures, a CPU velocity reference, edge variance, and Silk/engine residual-energy checks. CP-29/CP-30 establish capability; RC-13 remains partial. |
...
1. Close RC-13 quantitative gates: CPU velocity reference, settled edge variance, and Silk/engine previous-only residual energy.
