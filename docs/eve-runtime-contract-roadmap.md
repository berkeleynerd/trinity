...
| RC-04B | Reconstruct visible New Eden celestials | Active | RC-07 | The selected high-quality sun/planet graphs, CMF geometry, effects, and textures render through native `EvePlanet`. Meter-space placement and the standard planet warp-in control are implemented. Planet `40334264` now uses its exact `P_SandStorm_11` preset, FSD-selected height maps, and deterministic client parameters; isolated and combined captures validate the authored sandstorm palette. Atmosphere, cloud randomization/render-target preprocessing, exact-system-scale composition, flare, and god rays remain separate sub-gates. |
...
| CP-14 | New Eden authored planet surface | Accepted | Current client FSD resolves planet `40334264` to preset graphic `4321`, height graphics `3843`/`3903`, and their exact Black/DDS resources. The probe applies the client's `NormalHeight1`, `NormalHeight2`, and `Random=64` mutations before synchronous preparation; the isolated capture has the authored sandstorm palette. | The probe deliberately retains only the surface mesh. Client cloud scheduling, atmosphere, aurora, and data-driven FX remain unimplemented. |
...
1. Finish RC-04B planet atmosphere/cloud preprocessing and exact-system-scale
   controls; retain flare and god rays as separate visual gates.
